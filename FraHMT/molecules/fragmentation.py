...
def fragment_iterative(mol, min_length=3):
    ...
    fragindx=Chem.FragmentOnBonds(mol, bonds,dummyLabels=[(0, 0) for _ in range(len(bonds))])
    res = Chem.GetMolFrags(fragindx, asMols=True, sanitizeFrags=False)
    frags = [x for x in res if get_size(x) >= min_length]
    return frags
